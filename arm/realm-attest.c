				      ...
				      size_t *ptoken_size, bool report_success)
{
	...
	rc = claims_verify_token(buf, token_size, claimsp, report_success);
	free(local_claims);
	return rc;
}
...
{
	size_t token_size;
	struct attestation_claims *claims = malloc(sizeof(*claims));
